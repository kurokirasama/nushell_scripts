...
$env.PATH = (
  ...
  | prepend ('~/.local/share/omarchy/bin' | path expand)
...
# Locale & encoding defaults for subshells, Python runtime, and AppImages
if ($env.LANG? | is-empty) { $env.LANG = "en_US.UTF-8" }
if ($env.LC_ALL? | is-empty) { $env.LC_ALL = "en_US.UTF-8" }
