...
# $env.config.max_last_result_size = 1024mb # Stores up to 1024mb of last REPL pipeline output in $ans.last (uncomment in v0.115.0+)
...
let hooks = {
    pre_prompt: [
        ...
        },
        {
            ...
            code: "let m = (get-git-metrics); $env.GIT_BRANCH = $m.branch; $env.GIT_AHEAD = $m.ahead; $env.GIT_BEHIND = $m.behind; $env.GIT_STAGED = $m.staged; $env.GIT_MODIFIED = $m.modified; $env.GIT_DELETED = $m.deleted; $env.GIT_UNTRACKED = $m.untracked"
        },
        {
            condition: {not (".git" | path exists)},
            code: "$env.GIT_BRANCH = ''; $env.GIT_AHEAD = 0; $env.GIT_BEHIND = 0; $env.GIT_STAGED = 0; $env.GIT_MODIFIED = 0; $env.GIT_DELETED = 0; $env.GIT_UNTRACKED = 0"
        }
    ]
    pre_execution: [
        {||
