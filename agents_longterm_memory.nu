...
def get-consolidation-state [vault: path]: nothing -> any {
    ...
    let default_date = (date now) - 30day | date as iso-8601-utc
...
def set-consolidation-state [date: datetime, vault: path]: nothing -> nothing {
    let state_file = get-state-path $vault
    let state = { last_consolidation_date: ($date | date as iso-8601-utc) }
...
def resolve-unique-slug [slug: string, absolute_path: path, vault: path]: nothing -> any {
    ...
    # Case-insensitive scan of existing AGENTS_MEMORY directories
    let agents_memory = [$vault "AGENTS_MEMORY"] | path join
    let slug_lower = $slug | str lowercase
    if ($agents_memory | path exists) {
        let existing = ls $agents_memory
            | where type == dir
            | get name
            | where { |dir| ($dir | path basename | str lowercase) == $slug_lower }
            | first 1
        if ($existing | is-not-empty) {
            let existing_dir = $existing | first | path basename
            let note_path = get-note-path $existing_dir $vault
            if ($note_path | path exists) {
                ...
                if $data != null {
                    ...
                    if $note_abs_path == $path_str {
                        # Same project with different casing: reuse existing directory name
                        return $existing_dir
                    }
                }
            }
    ...
    # Fallback: scan ALL existing notes for a matching absolute_path, regardless of
    # slug derivation (e.g. product.md name vs directory basename produce different
    # slugs). Reuse the existing directory to prevent duplicates.
    if ($agents_memory | path exists) {
        let matching = ls $agents_memory
            | where type == dir
            | get name
            | each { |dir_path|
                let dir_name = $dir_path | path basename
                let note_path = get-note-path $dir_name $vault
                if ($note_path | path exists) {
                    let content = open --raw $note_path
                    let data = parse-frontmatter $content
                    if $data != null {
                        ...
                        if $note_abs_path == $path_str { $dir_name } else { null }
                    } else { null }
                } else { null }
            }
            | where { |r| $r != null }
...
def audition-skill [skill_path: path, draft_file: path] {
    ...
    loop {
        ...
        let analysis_context = [
            ...
            $"[SKILL_CONTENT]\n($skill_content)"
            $"[ITERATION_HISTORY]\n($iter_history_text)"
            "[SYNTHESIS_STANDARDS]\nEnsure skill has: 1) Explicit 'When NOT to Use' negative guidance, 2) Dependency verification rules before adding imports, 3) Zero-filler stop-on-completion mandate, 4) Test integrity rules (no masking errors)."
        ] | str join "\n"
...
export def agent-self-improve [] {
    ...
    let today = (date as iso-8601-date)
